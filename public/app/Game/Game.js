...
class Game {
    ...
    constructor() {
        ...
        ///
        this.score = 0; // Indica la puntuacion que se va adquiriendo
    ...
    start() {
        if (!this.started) {
            ...
            //score
            GAME_UI.marcador.update( GAME_UI.marcador.lifes, this.player.lives);
            GAME_UI.marcador.update( GAME_UI.marcador.score, this.score);
     ...
     */
    removeOpponent() {
        GAME_UI.gameBoard.removeChild(this.opponent.myImageContainer);
        
        //Creamos el oponente que sustituye al anterior
        if(this.score<6) this.opponent = new Opponent(this);
        else if(this.score<11){
            if(this.score===6) this.opponent=new Boss(this);
            else if(Math.random()>.8) this.opponent=new Boss(this);
            else this.opponent = new Opponent(this);
        }else if(this.score<21){
            if(this.score===11) this.opponent=new Boss(this);
            else if(Math.random()>.6)this.opponent=new Boss(this);
            else this.opponent = new Opponent(this);
        }else{
            if(Math.random()>.5)this.opponent=new Boss(this);
            else this.opponent = new Opponent(this);
        }
    ...
    checkCollisions() {
        ...
        if (impact || this.hasCollision(this.player, this.opponent)) {
            if(this.reduceVidas())this.player.die();
        }
        let killed = false;
    ...
    //para gestionar las colisiones cuando todavía tiene vidas
    reduceVidas(){
        if(!this.suspendido) {
            if(this.player.lives>0)GAME_UI.marcador.update( GAME_UI.marcador.lifes, --this.player.lives);
        }
        if(this.player.lives > 0){
            this.suspendido=true;
            this.player.myImage.src=this.player.myImageDead;
            setTimeout(() => {//en 2 s se renace
                game.player.myImage.src=game.player.myImageLive;
                game.suspendido=false;      
            }, 2000);
            return false;
        }else return true;
    }
