...
class Opponent extends Character {
     ...
     */
     constructor (game, cwidth, cheight, cx, cy, cspeed, cmyImage, cmyImageDead) {
        const height = cheight || OPPONENT_HEIGHT * game.width / FACTOR*ESCALA,
            ...
            myImage = cmyImage || OPPONENT_PICTURE,
            myImageDead = cmyImageDead || OPPONENT_PICTURE_DEAD;
    /*constructor(game) {
    ...
    die() {
        ...
        if (!this.dead) {
            //
            //document.getElementById('scoreli').innerHTML='Score: '+(++this.game.score);//se suma un punto a la puntuación adquirida al matar un oponente
            //GAME_UI.marcador.score.innerHTML='Score: '+(++this.game.score);
